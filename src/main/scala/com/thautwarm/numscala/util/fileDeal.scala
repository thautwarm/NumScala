...
import com.thautwarm.numscala.util.baseCalc.splitAt
...
object fileDeal {
  ...
  /* something like keywords which might look graceful */
  ...
  object xlsx {
    ...
    def unzipXlsxFile(input: String, output: String, dictArgs: immuMap[String, Any] = immuMap()): Unit = {
      ...
      val outputFix: String = if (output.endsWith("/")) output else output + "/"
        ...
        ((zipin: ZipInputStream) => {
          ...
          if (entry == pass) {
            ...
            ((file: File) => {
              ...
              if (filter != null && !filter(file.getAbsolutePath())) {
                pass
              }

              else if ((if (!file.getParentFile().exists()) {
                file.getParentFile().mkdirs(); true
              } else false)
                ||
                ...
                try {
                  ...
                  while (((readInstream: InputStream) => {
                    val readin = readInstream.read(); if (readin != (-1)) {
                      fileOut.write(readin); true
                    } else false
                  }) (input)) {
                    /*pass*/
                  }
